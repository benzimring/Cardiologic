...
open class WKTabBarController: UIViewController, WKTabBarControllerProtocol, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public var tabBarHeight: CGFloat = 52
    ...
    open func commonInit() {
        ...
        if #available(iOS 11.0, *) {
            container.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -tabBarHeight).isActive = true
        } else {
        ...
        let topLine = UIView.init(frame: CGRect(x: 0, y: collectionView.frame.height, width: view.frame.width, height: 1))
        topLine.backgroundColor = .lightGray
        collectionView.addSubview(topLine)
        
        
        let bottomLine = UIView.init(frame: CGRect(x: 0, y: tabBarHeight, width: view.frame.width, height: 1))
